...
from locotrack_pytorch.models.utils import Conv2dSamePadding
...
class EfficientNet_b0_feature_extractor_1_1(nn.Module):
  def __init__(self, model_name='efficientnet-b0'):
    super().__init__()
    model = EfficientNet.from_name(model_name=model_name)
    self.proj_1 = nn.Conv2d(in_channels=16, out_channels=64, kernel_size=1)
    self.proj_2 = nn.Conv2d(in_channels=24, out_channels=128, kernel_size=1)
    self.proj_3 = nn.Conv2d(in_channels=40, out_channels=256, kernel_size=1)
    self._conv_stem = model._conv_stem
    self._bn0 = model._bn0
    self._swish = model._swish
    self._blocks = nn.ModuleList(model._blocks[0:5]) 
  ...
  def forward(self, inputs):
    ...
    for idx, block in enumerate(self._blocks):
      ...
      if idx == 0:
        feature1 = self.proj_1(x)
        features_dict['reduction_1'] = feature1
      elif idx == 2:
        feature2 = self.proj_2(x)
        features_dict['reduction_2'] = feature2
      elif idx == 4:
        feature3 = self.proj_3(x)
        features_dict['reduction_3'] = feature3
        
    return features_dict


class EfficientNet_b0_feature_extractor_16_24_40(nn.Module):
  def __init__(self, model_name='efficientnet-b0'):
    super().__init__()
    model = EfficientNet.from_name(model_name=model_name)
    self._conv_stem = model._conv_stem
  ...
  def forward(self, inputs):
    ...
    for idx, block in enumerate(self._blocks):
      ...
      x = block(x, drop_connect_rate=drop_connect_rate)
      if idx == 0:
        features_dict['reduction_1'] = x
      elif idx == 2:
        features_dict['reduction_2'] = x
      elif idx == 4:
        features_dict['reduction_3'] = x
        
    return features_dict


class EfficientNet_256_feature_extractor_16_40_88(nn.Module):
  def __init__(self, model_name='efficientnet-256'):
    super().__init__()
    model = EfficientNet.from_name(model_name=model_name)
    self._conv_stem = model._conv_stem
    self._bn0 = model._bn0
    self._swish = model._swish
    self._blocks = nn.ModuleList(model._blocks[0:12]) 
    self.drop_connect_rate = model._global_params.drop_connect_rate
  def forward(self, inputs):
    features_dict = {}
    x = self._swish(self._bn0(self._conv_stem(inputs)))
    for idx, block in enumerate(self._blocks):
      ...
      if drop_connect_rate:
        drop_connect_rate *= float(idx) / len(self._blocks)
      x = block(x, drop_connect_rate=drop_connect_rate)
      if idx == 1:#16通道 [1, 16, 128, 128]
        features_dict['reduction_1'] = x
      elif idx == 7:#40通道 [1, 40, 32, 32]
        features_dict['reduction_2'] = x
      elif idx == 11:#88通道 [1, 88, 16, 16]
        features_dict['reduction_3'] = x
        
    return features_dict

class EfficientNet_b0_feature_extractor_16_40_112(nn.Module):
  def __init__(self, model_name='efficientnet-b0'):
    super().__init__()
    model = EfficientNet.from_name(model_name=model_name)
    self._conv_stem = model._conv_stem
    self._bn0 = model._bn0
    self._swish = model._swish
    self._blocks = nn.ModuleList(model._blocks[0:11]) 
    self.drop_connect_rate = model._global_params.drop_connect_rate
  def forward(self, inputs):
    features_dict = {}
    x = self._swish(self._bn0(self._conv_stem(inputs)))
    for idx, block in enumerate(self._blocks):
      ...
      if drop_connect_rate:
        drop_connect_rate *= float(idx) / len(self._blocks)
      x = block(x, drop_connect_rate=drop_connect_rate)
      if idx == 0:#16通道 [1, 16, 128, 128]
        features_dict['reduction_1'] = x
      elif idx == 4:#40通道 [1, 40, 32, 32]
        features_dict['reduction_2'] = x
      elif idx == 10:#112通道 [1, 112, 16, 16]
        features_dict['reduction_3'] = x
        
    return features_dict

class EfficientNet_b2_feature_extractor_16_48_120(nn.Module):
  def __init__(self, model_name='efficientnet-b2'):
    super().__init__()
    model = EfficientNet.from_name(model_name=model_name)
    self._conv_stem = model._conv_stem
    self._bn0 = model._bn0
    self._swish = model._swish
    self._blocks = nn.ModuleList(model._blocks[0:16]) 
    self.drop_connect_rate = model._global_params.drop_connect_rate
  def forward(self, inputs):
    features_dict = {}
    x = self._swish(self._bn0(self._conv_stem(inputs)))
    for idx, block in enumerate(self._blocks):
      ...
      if idx == 1:#16通道 [1, 16, 128, 128]
        features_dict['reduction_1'] = x
      elif idx == 7:#40通道 [1, 48, 32, 32]
        features_dict['reduction_2'] = x
      elif idx == 15:#112通道 [1, 120, 16, 16]
        features_dict['reduction_3'] = x
        
    return features_dict

class EfficientNet_b5_feature_extractor_40_64_176(nn.Module):
  def __init__(self, model_name='efficientnet-b5',in_channels=3,image_size=(384, 512)):
    super().__init__()
    model = EfficientNet.from_name(model_name=model_name,in_channels=3,image_size=(384, 512))
    self._conv_stem = model._conv_stem
    self._bn0 = model._bn0
    self._swish = model._swish
    self._blocks = nn.ModuleList(model._blocks[0:27]) 
    self.drop_connect_rate = model._global_params.drop_connect_rate
  def forward(self, inputs):
    features_dict = {}
    x = self._swish(self._bn0(self._conv_stem(inputs)))
    for idx, block in enumerate(self._blocks):
      ...
      if drop_connect_rate:
        drop_connect_rate *= float(idx) / len(self._blocks)
      x = block(x, drop_connect_rate=drop_connect_rate)
      if idx == 7:#40通道 [1, 40, 96, 128]
        features_dict['reduction_1'] = x
      elif idx == 12:#40通道 [1, 64, 48, 64]
        features_dict['reduction_2'] = x
      elif idx == 26:#112通道 [1, 176, 24, 32]
        features_dict['reduction_3'] = x

...
class CombinedFaceTracker:
    ...
    def get_face_masks(self, frame, erosion_kernel_size=5, erosion_iter=1, best_face_result=None):
        ...
        for face_result in best_face_result:
            ...
            # faceparsing_result = self.segmenter(frame).astype(np.float32)
    ...
    def track_forward(self, video, query):
        """
            ...
            Args:
                model: Locotrack模型
                video: [B,T, H, W, C] numpy array，已归一化的视频
                query: [B,N, 3] numpy array，查询点 [frame_idx, y, x]
                device: 设备
                radius: 跟踪半径
                threshold: 阈值
    ...
    def get_box_from_points(self, points,visibles,padding=0):
        """
        为所有可见的点生成包围框
    ...
    def post_process_by_displacement(self, tracks, visible, std_multiplier=2.0, min_valid_frames_ratio=0):
        """
        使用基于帧间位移的统计方法将每个点的异常帧标记为不可见。
        对所有点的所有位移进行处理，为每个点单独计算阈值。

        Args:
            tracks (np.array): 轨迹点数据，形状为 (num_points, num_frames, 2)。
            visible (np.array): 原始的可见性矩阵，形状为 (num_points, num_frames)。
            std_multiplier (float): 标准差乘数。位移超过 (中位数 + std_multiplier * 标准差)
                                    则被视为异常。值越小，过滤越严格。默认2.0。
            min_valid_frames_ratio (float): 每个点至少需要保留的有效帧比例。
                                            如果过滤后剩余帧数低于此比例，则该点所有帧都标记为不可见。

        Returns:
            np.array: 处理后的可见性矩阵。
        """
        num_points, num_frames, _ = tracks.shape
        visible_processed = np.copy(visible)
        
        # 1. 向量化计算所有帧间位移 (num_points, num_frames-1, 2)
        displacements_vectors = tracks[:, 1:, :] - tracks[:, :-1, :]
        
        # 2. 计算位移的欧几里得距离 (num_points, num_frames-1)
        displacements = np.linalg.norm(displacements_vectors, axis=2)
        ...
        for point_idx in range(num_points):
            # 获取该点的所有位移
            point_displacements = displacements[point_idx]
            
            # 如果位移数量太少，跳过处理
            if len(point_displacements) < 3:
                continue
            
            # 4. 计算该点位移的中位数
            point_median_displacement = np.median(point_displacements)
            
            # 5. 计算该点每个位移到中位数的距离
            distances_to_median = np.abs(point_displacements - point_median_displacement)
            
            # 6. 计算该点距离的均值和标准差，用于计算阈值
            mean_distance = np.mean(distances_to_median)
            std_distance = np.std(distances_to_median)
            threshold = mean_distance + std_multiplier * std_distance
            
            # 7. 找出该点的异常位移
            outlier_displacements = distances_to_median > threshold
            
            # 调试信息：打印前几个点的统计信息
            if point_idx < 3:
                print(f"点{point_idx}: 中位数={point_median_displacement:.2f}, 均值距离={mean_distance:.2f}, 标准差={std_distance:.2f}, 阈值={threshold:.2f}")
                print(f"  异常位移数量: {np.sum(outlier_displacements)}/{len(outlier_displacements)}")
                print(f"  最大距离: {np.max(distances_to_median):.2f}")
            
            # 8. 将异常位移对应的"当前帧"（第i+1帧）标记为不可见
            frames_to_remove = np.zeros(num_frames, dtype=bool)
            frames_to_remove[1:] = outlier_displacements
            
            # 9. 应用最小有效帧比例规则
            original_visible_count = np.sum(visible[point_idx])
            if original_visible_count == 0:
                continue
                
            removed_count = np.sum(frames_to_remove & visible[point_idx])
            remaining_count = original_visible_count - removed_count
            min_required_frames = int(original_visible_count * min_valid_frames_ratio)
            
            if remaining_count < min_required_frames:
                visible_processed[point_idx, :] = False
                total_removed += original_visible_count
            else:
                visible_processed[point_idx, frames_to_remove] = False
                total_removed += removed_count
        
        print(f"后处理完成: 总共移除了 {total_removed} 个可见点")
        return visible_processed
    
    def post_process_by_frame_visibility(self, visible, min_visible_ratio=1/3):
        """
        按帧后处理：如果某帧可见点数少于总点数的指定比例，将该帧所有点设为不可见
        
        Args:
            visible (np.array): 可见性矩阵，形状为 (num_points, num_frames)
            min_visible_ratio (float): 最小可见点比例阈值，默认1/3
        ...
        min_visible_count = int(num_points * min_visible_ratio)
        
        for frame_idx in range(num_frames):
            # 统计当前帧可见点数
            visible_count = np.sum(visible[:, frame_idx])
            
            # 如果可见点数少于阈值，将该帧所有点设为不可见
            if visible_count < min_visible_count:
                visible_processed[:, frame_idx] = False
    ...
    def process_gap_with_locotack(self, frames, tmp_result, gap_start, gap_end, buffer=10, target_size_x=256, target_size_y=256, grid_size=8):
        ...
        # 后处理
        visible = self.post_process_by_displacement(tracks, visible,std_multiplier=1,min_valid_frames_ratio=0)
        visible = self.post_process_by_frame_visibility(visible, min_visible_ratio=0.4)
    ...
    def process_video(self, video_path, output_path, buffer_frames=10, target_size_x=256, target_size_y=256):
        ...
        #    使用集合(set)来自动去重
        all_ids = set()
        for frame_faces in tmp_result:
            for face in frame_faces:
                all_ids.add(face.face_id)
        
        # 将ID排序，以确保每次运行的输出顺序都一致
        sorted_unique_ids = sorted(list(all_ids))
        total_frames = len(tmp_result)

        # 2. 创建一个字典来存储每个ID的追踪结果
        #    键(key)是 face_id，值(value)是一个预先用空列表填满的、与视频等长的列表
        results_by_id = {
            face_id: [[] for _ in range(total_frames)] for face_id in sorted_unique_ids
        }
        
        # 3. 遍历原始结果，将数据填充到对应的ID和帧位置
        #    使用 enumerate 可以同时获得帧的索引和内容
        for frame_index, frame_faces in enumerate(tmp_result):
            for face in frame_faces:
                # face.face_id 告诉我们这是哪个人
                # frame_index 告诉我们这是哪一帧
                # 我们把这个 face 的信息，放到这个人专属列表的正确帧位置上
                # 结果需要用列表包装，以符合 [FaceTrackingResult(...)] 的格式
                results_by_id[face.face_id][frame_index] = [face]

        # 4. 将字典中的值（也就是每个人的结果列表）提取出来，形成最终的列表
        #    通过遍历排好序的ID列表来提取，保证了最终输出的顺序
        all_persons_results = [results_by_id[face_id] for face_id in sorted_unique_ids]
        ...
        # 为每个槽位单独检测和处理跟踪空白段
        all_gap_tracks_data = {}  # key: slot_idx, value: {(gap_start, gap_end): tracks_data}
        
        for person_idx, person_tmp_result in enumerate(all_persons_results):
            # 检测该槽位的跟踪空白段
            gaps = self.detect_tracking_gaps(person_tmp_result)
            ...
            # 处理该槽位的每个空白段
            person_gap_tracks_data = {}
            for gap_start, gap_end in gaps:
                tracks_data = self.process_gap_with_locotack(
                    frames, person_tmp_result, gap_start, gap_end, 
                    buffer=buffer_frames, target_size_x=target_size_x, target_size_y=target_size_y
                )
                if tracks_data is not None:
                    person_gap_tracks_data[(tracks_data['gap_start'], tracks_data['gap_end'])] = tracks_data
            
            if person_gap_tracks_data:
                all_gap_tracks_data[person_idx] = person_gap_tracks_data
        ...
        for frame_idx, frame in enumerate(frames):
            ...
            # 为每个人处理跟踪结果
            for person_idx in range(len(sorted_unique_ids)):
                # 获取当前人在当前帧的原始跟踪结果
                person_frame_results = all_persons_results[person_idx][frame_idx]

                # 检查当前帧是否在该人的跟丢段内
                in_gap = False
                current_gap_data = None
                
                if person_idx in all_gap_tracks_data:
                    for (gap_start, gap_end), tracks_data in all_gap_tracks_data[person_idx].items():
                ...
                if not in_gap:
                    ...
                    annotated_frame = self.draw_face_tracking_results(annotated_frame, person_frame_results)
                    ...
                    if 0 <= local_frame_idx < current_gap_data['tracks'].shape[0]:
                        bbox = self.get_box_from_points(
                            current_gap_data['tracks'][local_frame_idx],
                            current_gap_data['visible'][local_frame_idx]
                        )
                        ...
                        if bbox is not None:
                            ...
                            # 获取当前特征
                            if len(person_frame_results) > 0:
                                cur_feat = person_frame_results[0].face_feature
                            else:
                            ...
                            cv2.rectangle(annotated_frame, 
                                        ...
                                        (int(bbox_list[0]+bbox_list[2]), int(bbox_list[1]+bbox_list[3])), 
                                        (0,0, 255), 2)
                            ...
                            # 绘制面部特征框
                            cv2.rectangle(annotated_frame, 
                                        (int(cur_lmk_xy_box[0]), int(cur_lmk_xy_box[1])), 
                                        (int(cur_lmk_xy_box[0]+cur_lmk_xy_box[2]), 
                                        int(cur_lmk_xy_box[1]+cur_lmk_xy_box[3])), 
                                        (255,0, 0), 2)
    ...
    def batch_process_videos(self,input_base_path,output_base_path,buffer_frames=10, target_size_x=256, target_size_y=256, face_models_path="face_models"):
        # 输入和输出路径
        ...
        # 查找所有视频文件（支持常见视频格式）
        video_extensions = ['*.mp4', '*.avi', '*.mov', '*.mkv', '*.flv', '*.wmv','.MP4']
        video_files = []
        ...
        video_files = [
            "/root/group-trainee/zyp/project1_7_15/locotrack/input_test/W9371.MP4"
        ]
        begin_flag=False
...
# #     "/root/group-trainee/zyp/pyscenedetec_seg/group2/不同人种/模糊/模糊_scene_001.mp4"
        ...
        # "/root/zyp/output/lcootrack/combined_locotrack_efficientnet_faceparsing_output/test/goodcase/模糊.mp4"
        "/root/group-inspect2-data/视频需求/测试数据/视频人脸跟踪测试集/人脸跟踪-场景切分 250923/运动/10仰头侧脸 遮挡.mp4",
        "/root/group-inspect2-data/视频需求/测试数据/视频人脸跟踪测试集/人脸跟踪-场景切分 250923/短剧/1转头/1转头22.mov",
        "/root/group-inspect2-data/视频需求/测试数据/视频人脸跟踪测试集/人脸跟踪-场景切分 250923/运动/12侧脸 遮挡.mp4",
        "/root/group-inspect2-data/视频需求/测试数据/视频人脸跟踪测试集/人脸跟踪-场景切分 250923/电影/7侧脸/7侧脸9.mov",
        "/root/group-inspect2-data/视频需求/测试数据/视频人脸跟踪测试集/人脸跟踪-场景切分 250923/短一镜/3 快速移动 侧脸.mp4",
        "/root/group-inspect2-data/视频需求/测试数据/视频人脸跟踪测试集/人脸跟踪-场景切分 250923/婚礼/5转头 多人 运镜/5转头 多人 运镜19.mov",
        "/root/group-inspect2-data/视频需求/测试数据/视频人脸跟踪测试集/人脸跟踪-场景切分 250923/短剧/1转头/1转头22.mov"
        # "/root/group-inspect2-data/视频需求/测试数据/视频人脸跟踪测试集/人脸跟踪-场景切分 250923/短一镜/5.mp4",
        # "/root/group-inspect2-data/视频需求/测试数据/视频人脸跟踪测试集/人脸跟踪-场景切分 250923/短一镜/11.mp4",
        # "/root/group-inspect2-data/视频需求/测试数据/视频人脸跟踪测试集/人脸跟踪-场景切分 250923/电影/6多人脸/6多人脸6.mov",
        # "/root/group-inspect2-data/视频需求/测试数据/视频人脸跟踪测试集/人脸跟踪-场景切分 250923/运动/25 多人 遮挡.mp4",
        # "/root/group-inspect2-data/视频需求/测试数据/视频人脸跟踪测试集/人脸跟踪-场景切分 250923/短一镜/7手遮挡.mp4"
 ]
        for video_path in video_files:
            ...
            # # 只测试最后一个视频
            # if video_path != specific_videos_to_process[-1] and not begin_flag:
            #     print(f"Skipping {video_path}")
            #     continue
...
def main():
    # 配置参数
    input_base_path ="/root/group-trainee/zyp/project1_7_15/locotrack/input_test/"
    output_base_path = "/root/group-trainee/zyp/project1_7_15/locotrack/output/b5/"
    locotrack_efficientnet_checkpoint = "locotrack_pytorch/small_efficentnet_b5_40_64_176_mlp_input_features_854_chekpoint/epoch=0-step=285000.ckpt"
    locotrack_efficientnet_model_size="small_efficentnet_b5_40_64_176_mlp_input_features_854"
    buffer_frames=10
    target_size_x=512
    target_size_y=384
    face_models_path="face_models"
    ...
    # 处理视频
    tracker.batch_process_videos(input_base_path, output_base_path, buffer_frames=buffer_frames, target_size_x=target_size_x, target_size_y=target_size_y, face_models_path=face_models_path)
